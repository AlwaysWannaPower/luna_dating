    ...
    <div class="flex flex-col items-center justify-center px-4 py-12 sm:py-20">
        ...
        <div class="max-w-3xl mx-auto text-center mb-12 animate-fade-in">
            <h1 class="text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight mb-6">
                <span class="block text-text-primary dark:text-dark-text">{{ __('Найди свою') }}</span>
                <span class="block bg-gradient-to-r from-luna-primary via-luna-accent to-luna-primary bg-clip-text text-transparent">{{ __('вторую половинку') }}</span>
            </h1>
            <p class="text-lg sm:text-xl text-text-secondary dark:text-dark-text-muted max-w-2xl mx-auto leading-relaxed">
                {{ __('Luna — это место, где начинаются настоящие истории.') }}<br class="hidden sm:block">
                {{ __('Тысячи людей уже нашли друг друга. Может быть, следующим будешь ты?') }}
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-4xl mb-12">
            <div class="bg-bg-primary dark:bg-dark-bg-card rounded-2xl p-6 shadow-sm border border-border dark:border-dark-border hover:shadow-md hover:border-luna-primary/30 dark:hover:border-luna-primary/30 transition-all duration-300 group">
                ...
                <h3 class="text-lg font-semibold mb-2 text-text-primary dark:text-dark-text">{{ __('Создай профиль') }}</h3>
                <p class="text-sm text-text-secondary dark:text-dark-text-muted">{{ __('Расскажи о себе и своих интересах. Фотографии помогут привлечь внимание.') }}</p>
            </div>
            ...
            <div class="bg-bg-primary dark:bg-dark-bg-card rounded-2xl p-6 shadow-sm border border-border dark:border-dark-border hover:shadow-md hover:border-luna-primary/30 dark:hover:border-luna-primary/30 transition-all duration-300 group">
                ...
                <h3 class="text-lg font-semibold mb-2 text-text-primary dark:text-dark-text">{{ __('Ищи знакомства') }}</h3>
                <p class="text-sm text-text-secondary dark:text-dark-text-muted">{{ __('Просматривай анкеты, ставь лайки и находи интересных людей рядом.') }}</p>
            </div>
            ...
            <div class="bg-bg-primary dark:bg-dark-bg-card rounded-2xl p-6 shadow-sm border border-border dark:border-dark-border hover:shadow-md hover:border-luna-primary/30 dark:hover:border-luna-primary/30 transition-all duration-300 group">
                ...
                <h3 class="text-lg font-semibold mb-2 text-text-primary dark:text-dark-text">{{ __('Общайся') }}</h3>
                <p class="text-sm text-text-secondary dark:text-dark-text-muted">{{ __('Начни общение с теми, кто понравился тебе. Взаимные симпатии ведут к чату.') }}</p>
            </div>
        </div>
        ...
        <div class="mt-16 pt-8 border-t border-border dark:border-dark-border w-full max-w-3xl">
            <div class="grid grid-cols-3 gap-8 text-center">
                <div>
                    ...
                    <div class="text-xs sm:text-sm text-text-secondary dark:text-dark-text-muted mt-1">{{ __('Пользователей') }}</div>
                </div>
                <div>
                    <div class="text-2xl sm:text-3xl font-bold text-luna-primary">1K+</div>
                    <div class="text-xs sm:text-sm text-text-secondary dark:text-dark-text-muted mt-1">{{ __('Пар каждый день') }}</div>
                </div>
                <div>
                    <div class="text-2xl sm:text-3xl font-bold text-luna-primary">50+</div>
                    <div class="text-xs sm:text-sm text-text-secondary dark:text-dark-text-muted mt-1">{{ __('Городов') }}</div>
                </div>
            </div>
        </div>
    </div>
