<x-guest-layout>
    ...
    <div class="mb-6 text-center">
        <h2 class="text-2xl font-bold text-text-primary dark:text-dark-text">{{ __('С возвращением!') }}</h2>
        <p class="text-text-secondary dark:text-dark-text-muted mt-1">{{ __('Войдите, чтобы продолжить') }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <div class="flex items-center justify-between">
            <label for="remember_me" class="inline-flex items-center cursor-pointer">
                <input id="remember_me" type="checkbox" class="rounded border-text-muted dark:border-dark-border text-luna-primary shadow-sm focus:ring-luna-primary focus:ring-2 focus:ring-luna-primary/20" name="remember">
                <span class="ms-2 text-sm text-text-secondary dark:text-dark-text-muted">{{ __('Запомнить меня') }}</span>
            </label>

            @if (Route::has('password.request'))
                <a class="text-sm text-luna-primary hover:text-luna-primary-hover hover:underline focus:outline-none focus:underline transition-colors duration-200" href="{{ route('password.request') }}">
                    {{ __('Забыли пароль?') }}
                </a>
            @endif
        </div>
    ...
    <p class="mt-6 text-center text-sm text-text-secondary dark:text-dark-text-muted">
        {{ __('Нет аккаунта?') }}
        <a href="{{ route('register') }}" class="font-semibold text-luna-primary hover:text-luna-primary-hover hover:underline transition-colors duration-200">
